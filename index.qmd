...
json_files <- c(
  "~/Desktop/25- 26 Fall MS UNL/STAT850 Computing Tools/251104 Week11/list-processing-themore0612/drwho-210.json",
  "~/Desktop/25- 26 Fall MS UNL/STAT850 Computing Tools/251104 Week11/list-processing-themore0612/drwho-766.json",
  "~/Desktop/25- 26 Fall MS UNL/STAT850 Computing Tools/251104 Week11/list-processing-themore0612/drwho-72724.json"
)

drwho_list <- map(json_files, ~ read_json(.x))
names(drwho_list) <- basename(json_files)
str(drwho_list,1)
head(drwho_list$`drwho-210.json`,1)
```
---
...
```{r}
library(jsonlite)
drwho210 <- read_json("drwho-210.json")
prettify(toJSON(drwho210))
```
----
- episode
  - id
...
Yes. Some redundancy exists:
    1. season and number – implied by the episode’s position in the season list.
    2. url vs _links$self$href – both link to the episode.
	3. _links$show$name – repeated in every episode if show info is stored at the top.
...
- type
- image (optional)
- year (optional, extracted from airdate)
---
...
---
1.	Read JSON files into R.
2.	Inspect structure with str() or names().
3.	Flatten nested fields (rating, image, _links) and convert single-item arrays to scalars.
4.	Extract episode data: one row per episode with show, season, episode_number, title, airdate, runtime, type, rating.
5.	Combine all files into one table.
6.	Clean/format: convert dates, numeric types, handle missing values.
...
extract_episodes <- function(show_json) {
  map_df(show_json, function(ep) {
    tibble(
      show = ep$`_links`$show$name[[1]],
      season = as.integer(ep$season[[1]]),
      episode_number = as.integer(ep$number[[1]]),
      title = ep$name[[1]],
      airdate = as.Date(ep$airdate[[1]]),
      runtime = as.integer(ep$runtime[[1]]),
      rating = as.numeric(ep$rating$average[[1]]),
      type = ep$type[[1]],
      image = ep$image$medium[[1]]
    )
  })
}

drwho_tables <- map(json_files, ~ {
  show_data <- read_json(.x)
  extract_episodes(show_data)
})

names(drwho_tables) <- c("drwho210", "drwho766", "drwho72724")
lapply(drwho_tables, head, 5)
...
all_episodes <- bind_rows(drwho_tables, .id = "series")
all_episodes <- all_episodes %>%
  mutate(series = factor(series, levels = c("drwho-210", "drwho-766", "drwho-72724")))

all_episodes <- all_episodes %>%
  arrange(series, season, episode_number) %>%
  group_by(series, season) %>%
  mutate(days_between = as.numeric(airdate - lag(airdate))) %>%
  ungroup()

ggplot(all_episodes %>% filter(!is.na(days_between)),
       aes(x = episode_number, y = days_between, color = series)) +
  geom_point() +
  geom_line() +
  facet_wrap(~ series + season, scales = "free_x") +
  labs(
    title = "Days Between Air Dates of Consecutive Doctor Who Episodes",
    x = "Episode Number",
...
The plot shows that episode spacing varies by series and season: older series (e.g., drwho-210) tend to have shorter, more consistent gaps between episodes, while newer series (drwho-766 and drwho-72724) exhibit longer and more irregular gaps, particularly around specials or season breaks. Large spikes in days_between indicate break/pauses or multi-part episodes. A minor data quality issue is the NA values for the first episode of each season, which are expected since there’s no prior episode to calculate a gap.
...
json_files <- list.files(
  path = "~/Desktop/25- 26 Fall MS UNL/STAT850 Computing Tools/251104 Week11/list-processing-themore0612",
  pattern = "*.json",
  full.names = TRUE
)

extract_episodes <- function(file_path) {
  show_data <- read_json(file_path)
  map_df(show_data, function(ep) {
    tibble(
      show = ep$`_links`$show$name[[1]],
      season = as.integer(ep$season[[1]]),
      episode_number = as.integer(ep$number[[1]]),
...
all_files <- map_df(json_files, extract_episodes, .id = "series")
...
extract_clean_episodes <- function(file_path) {
  show_data <- read_json(file_path)
...
whoverse <- map_df(json_files, extract_clean_episodes, .id = "series") %>%
  mutate(
...
Observations & Conclusions
	1.	Target Audience:
	    - Episodes airing before 9:00pm are generally aimed at family or younger audiences.
        - Episodes airing during or after the watershed (9:00pm+) are likely intended for older teens or adults.
        - Classic series (210) often aired earlier in the evening, while modern series sometimes push closer to the watershed, suggesting a slightly older audience for later episodes.
        
	2.	Missing Airtimes:
        - Some older or special episodes may not have airtimes recorded, either because historical records are incomplete or because specials were released in non-standard formats.
...
extract_show_urls <- function(file) {
  data <- fromJSON(file)
  sapply(data, function(item) item[["_links"]][["show"]][["href"]])
}

show_urls <- unlist(lapply(json_files, extract_show_urls))

get_show_data <- function(url) {
  fromJSON(url)
}

all_shows <- lapply(show_urls, get_show_data)
sapply(all_shows, function(show) show$name)
```
----
